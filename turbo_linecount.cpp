...
//  cpucount = 1;
  ...
  if (parameters)
  {
    if (parameters->buffersize != (unsigned long)-1)
    {
...
//    printf("%p %lld %lld\n",mem, mapsize, curoffset);
    ...
    if ((curoffset + mapsize) == (unsigned long long)m_filesize)
    {
...
bool CLineCount::countLines(std::vector<std::vector<uint64_t>> & lineoffsets, tlc_linecount_t & linecount, bool offsets)
{
...
//  printf("act: %d\n",m_actual_thread_count);
  ...
  if (printOffsets) {
    lineoffsets = m_threadlineoffsets;
  }
